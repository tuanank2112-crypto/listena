...
**Cảnh báo cấu hình (root review 2026-09-17 — Đã xử lý):** loại database phải suy ra từ `resolveDatabaseConfig()`, KHÔNG đọc `process.env.DATABASE_URL` trực tiếp. Nhánh hosted Turso lấy `TURSO_DATABASE_URL` + `TURSO_AUTH_TOKEN` và không đọc `DATABASE_URL`, nên mọi phép kiểm dựa vào biến đó sẽ chạy nhánh local trên hosted. Đã xử lý dứt điểm tại `shouldSerializeLocally()` trong `src/lib/libsql-batch.ts` (finding F1/F2), tắt mutex tiến trình trên Turso hosted.
...
Plan10 added Attempt.clientAttemptId/requestHash, ReviewLog.clientReviewId/requestHash, VocabularyMastery.revision and LessonCreationRequest. Plan11 migration `20260916120000_plan11_integrity_receipts` (committed in 9325ca2) adds Attempt.resultJson/enrichmentState(default NOT_REQUESTED)/enrichmentLeaseId/enrichmentLeaseExpiresAt and ReviewLog.resultJson; it is NOT applied to prisma/dev.db (Plan12 P120 qualified on isolated fixtures). Write paths move to `withLibSqlWriteTransaction` (read-in-tx, CAS rowsAffected guard). Attempt writes VocabularyMastery only for error words (revision +1 per write); correct answers leave revision unchanged. Old rows with null resultJson replay as 409 LEGACY_RESULT_UNAVAILABLE.
...
English uses WebSpeech behind the Plan14 curated layer: `toLearningSessionDto` adds a server-built `voiceScript` to AI turns (never persisted; derived from `contentJson`), and `POST /api/voice/pronunciation` appends `VOICE_PRACTICE` SYSTEM turns (`{event, value%}`) to the session ledger only for lines the AI modelled. It writes no LearningEvidence/mastery. Optional Vietnamese Next/Python boundary requires user/shared-key auth, private no-store caching, loopback sidecar and speed1.0 only. Kokoro removed. No speech ability inference from text evidence.
