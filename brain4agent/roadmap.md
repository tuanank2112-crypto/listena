...
## Ưu tiên tiếp theo
1. Owner installs a freshly rotated `KIRAAI_API_KEY` as a Worker secret and records one bounded, persisted personalized-lesson smoke; then observe quota/cost/latency before expanding live generation. OpenAI remains an explicit alternate path, not a Kira URL override.
