...
- 2026-09-16 12:41+07: Post-merge GitHub CI evidence was corrected without changing hosted gates. npm 11 on Node 24 exposed an incomplete npm-generated lock graph (`@emnapi/core@1.11.3` and `@emnapi/runtime@1.11.3` missing); the lockfile was regenerated by npm rather than hand-edited. The job-level `MIGRATION_WRITE_MODE=disabled` also leaked into isolated Playwright and caused mutation APIs to return 503, so `playwright.config.ts` now explicitly enables writes only for its guarded temporary SQLite database. GitHub Actions run `35060225165` at commit `9a48793` passed every standard CI step, including clean `npm ci`, 437/437 Vitest coverage tests, Python 7/7, Next build, offline quality and isolated E2E 20/20. Preview/Production remain fenced and unchanged.
