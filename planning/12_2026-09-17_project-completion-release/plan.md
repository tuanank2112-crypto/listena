...
- Status: IN PROGRESS — P120–P126 local ✅, production cutover đã làm 17/09 16:30 (mail/AI hosted còn mở); rà logic toàn repo 17/09 tối mở 5 P1 chờ Plan13
...
- 2026-09-17 12:20+07: Worker thực thi xử lý toàn bộ 7 finding theo `docs/ROOT_CODE_REVIEW_2026-09-17_P120-P126.md`: F1 & F2 sửa `shouldSerializeLocally()` dùng `resolveDatabaseConfig()` kèm 4 test hồi quy; F3 nối `clearOwnerIntents` vào đăng xuất và đổi tài khoản trong `app-shell.tsx` kèm test (6/6 PASS); F4 bỏ fallback `"anonymous"` ở 4 file UI; F5/F6/F7 sửa câu chữ changelog/ledger; hạ version `package.json` về `0.5.0`. Cổng local đạt: 494 test/93 files vitest, 24/24 Playwright E2E, type-check 0 lỗi, lint 0 lỗi/32 cảnh báo, build 42 routes PASS, eval 12/12 PASS.
- 2026-09-17 18:45+07: User báo lại "Gia sư AI hiện chưa sẵn sàng" và yêu cầu kiểm tra não. Root boot não (BRN-011 tự sửa), xác minh `.env` + `ai:doctor --probe` OK, dev local không chạy nên lỗi là trên production. Tái hiện tại local bằng chính provider: sinh bài học cá nhân hoá (2.200 token) cùng input lần 1 xong 25s, lần 2 timeout 50s đúng thông báo; đo không cắt 8 lần (claude-sonnet-4-6 + deepseek-v4-flash) thì 3/8 hoàn tất dưới 50s, 1 lần 74s, 4 lần gateway Vyce treo ~125s trả HTML. Mission 4/4 OK 5–10s. Kết luận: không phải cấu hình; đường Bài AI riêng chạy đồng bộ trong 60s không chịu nổi phân bố này; 24f380a (50s) chưa thấy deploy và cũng không đủ. Không sửa code, không deploy. Chờ user chọn: async generation (đề xuất) / maxDuration 300 + Fluid / đổi provider cho generation dài / deploy 24f380a.
- 2026-09-17 18:50+07: User quyết định **tăng timeout lên ~3 phút** vì Vyce là API gateway. Root sửa: `DEFAULT_TIMEOUT_MS`/`MAX_TIMEOUT_MS` 50s/60s → 180s/180s trong `kira-chat-completions-provider.ts`; `maxDuration` 60 → 200 ở 5 route AI (learning-sessions, turns, tutor, personalized-lessons, teacher/generate-lesson); `GENERATION_STALE_MS` 90s → 210s để row GENERATING không bị chiếm lại khi request gốc còn chạy. Giữ `AI_REQUEST_PENDING_LEASE_MS` 30s (lease chỉ chống bấm dồn; nâng lên 200s sẽ khoá học viên nếu function bị giết trước khi settle). Điều kiện hosted: Vercel phải cho phép 200s (Hobby cần Fluid compute, trần 300s), nếu không build/deploy sẽ từ chối — kiểm ở lần deploy. Lưu ý: khi gateway treo ~125s thì học viên chờ tới lúc gateway trả HTML rồi mới nhận lỗi typed; không còn bị ta cắt oan.

- 2026-09-17 21:50+07: User: "chỉ dùng model trong Vyce, kiraai bỏ đi" + "check lại logic toàn bộ repo, não không đủ thông tin". Root (a) gỡ provider Kira: đổi tên thành `VyceChatCompletionsProvider` (`src/server/ai/vyce-chat-completions-provider.ts`), allowlist chỉ `https://vyceai.com/v1`, model mặc định `claude-sonnet-4-6`, env `AI_PROVIDER=vyce` + `VYCE_API_KEY/VYCE_MODEL/VYCE_BASE_URL`; `AI_PROVIDER=kira`/`KIRAAI_API_KEY` còn sót → fail closed + log "stale" (test hồi quy); sửa luôn `eval/learning-run.ts` đọc nhầm `KIRA_API_KEY`. Type-check 0, 13 file test AI 105/105. **Vercel env chưa đổi tên — user phải đổi trước deploy** (runbook §2.5). (b) Rà logic toàn repo bằng 6 agent read-only theo phân hệ, root kiểm chứng P1 trên code: [Root logic review 2026-09-17](../../docs/ROOT_LOGIC_REVIEW_2026-09-17_FULL_REPO.md) — 5 P1 (mastery chia /100 sai, đáp án lộ qua `Exercise.metadata`, CEFR tăng bậc mỗi attempt, session ACTIVE không evidence + AI hỏng thì kẹt, enumeration theo thời gian ở route mail), 17 P2, ~30 P3. Chưa sửa finding nào; đề xuất Plan13 (SPEC package) vì vượt hotfix.

## Quyết định bị thay thế

- "Provider timeout 45s→50s, sized against the 60s route budget" (24f380a, 17:4x) → **thay bằng** 180s + `maxDuration = 200` (18:50) sau khi đo phân bố thật cho thấy 60s không đủ; Plan07 SPEC-P71 "maxDuration = 60" cũng bị thay thế, không sửa ngược.
...
- [x] P126: Runbook sự cố [docs/RUNBOOK_INCIDENT.md](docs/RUNBOOK_INCIDENT.md) hoàn thành; bài drill backup/restore thật qua `scripts/verify-backup-restore.ts` đạt 100% data fidelity; `npm audit --omit=dev` kiểm tra; tài liệu và version 0.5.0 (hướng tới 1.0.0 sau khi cutover đạt) đồng bộ toàn dự án.
- [x] Phê duyệt của người dùng để thực hiện commit và kích hoạt cutover hosted trên Vercel/Turso. — cutover 16:30, mail/AI hosted còn mở (ghi nhận Plan13 P134, 2026-09-17).
