...
export type LearningSessionPlayerAction =
  | { type: "LOAD_START" }
  | { type: "LOAD_SUCCESS"; session: PublicLearningSession; now: number }
  | { type: "LOAD_FAILURE"; error: string }
  | { type: "SET_DRAFT"; value: string }
  | { type: "COUNT_HINT" }
  | { type: "COUNT_REPLAY" }
  | { type: "SUBMIT_START"; submission: TurnSubmission }
  | { type: "SUBMIT_SUCCESS"; session: PublicLearningSession; now: number }
  | { type: "SUBMIT_FAILURE"; error: string }
  | { type: "COMPLETE_START" }
  | { type: "COMPLETE_SUCCESS"; session: PublicLearningSession }
...
export function learningSessionPlayerReducer(
  ...
  switch (action.type) {
    ...
    case "COMPLETE_SUCCESS":
      return { ...state, phase: "completed", session: action.session, error: "", pendingSubmission: null };
